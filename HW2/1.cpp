...
int main( )
{
    ...
    if (yourAmount == myAmount)
        ...
        //NEW CODE FOR TESTING *Note <, >, =>, =< have essentially the same copy pasted code
        if(yourAmount < myAmount) {
    ...
    int p = 10;
    cout << p << "% of your money is " << yourAmount.percent(p) << endl;
...
const Money Money::percent(int percentFigure) const {
  Money percentMoney;
  percentMoney.dollars = dollars * percentFigure * 0.01;
  percentMoney.cents = cents * percentFigure * 0.01;
  return percentMoney;
}
